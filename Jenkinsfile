...
void setBuildStatus(String message, String state) {
	step([
		...
		reposSource: [$class: 'ManuallyEnteredRepositorySource', url: 'https://github.com/lapicidae/mariadb-epglv'],
	...
	stages {
		...
		stage('Version') {
			steps{
				...
				script {
					VERSION = sh(returnStdout: true, script: 'docker run --rm --entrypoint "" $registry:$BUILD_NUMBER cat /etc/lsb-release | grep -i codename | cut -d = -f 2').trim()
				}
